...
!------------------------------------------------
! Main : Cross-validation for Cartesian grid
!---------------------------------------------
program tomo_cart_cv
 use InternPro
 use TomoPro
 implicit none
...
character (len=100) :: Abuffname, Rbuffname, sname, cvname 
real (kind=double) :: ds, xcar(grid_sz)
real (kind=double), dimension(grid_sz,grid_sz,grid_sz) :: xp,yp,zp, rdis
integer :: i,j,k, isz, ii, jj, mm, nn, nnz
integer, dimension(:), allocatable:: tdis,wdis,iw,jw,kw
integer :: ndis,ind 
integer :: allocatestatus, Openstatus, open_status
type (SparseMat) :: Amat, Rmat, S_sprs, B_sprs
real :: sp, tol, av_erp
...
real, dimension(:,:), allocatable :: mxsolu_t, mxsolu
real, dimension(grid_sz,grid_sz,grid_sz) :: nsolu
integer :: indp(1), ind0(1), nsz
integer :: sav_res,sav_iter,resume,is_nwt
logical :: exist
...
100 continue
  ...
  do 200 j=1,grid_sz
  do 200 i=1,grid_sz
       if (rdis(i,j,k) >= grid_Rin-2*ds .and. rdis(i,j,k) <= grid_Rout+2.0*ds) then
...
! print*,'ndis=',ndis
 ...
 write(snum,'(I3)') grid_sz 
 gridname='sz'// trim(adjustL(snum))
 ...
 print*,'Amean in doublePrec=',sum(dble(Amat%A))/Amat%asiz
 print*,'min max ydata=',minval(ydata),maxval(ydata) 
 print*,'Mean Ydata=',sum(ydata)/size(ydata)
 ...
 print*,'Rmat: mm,nn,nnz=', Rmat%m, Rmat%n, Rmat%asiz
 print*,'min max R=',minval(Rmat%A),maxval(Rmat%A)

  nsz=Amat%n

 ALLOCATE(xsolu(nsz),xsolu_p(nsz), mxsolu_t(nsz,msz),mxsolu(nsz,msz),erp(mfold), &
 ...
 DO i=1, msz
   ...
   DO j=1,mfold
     ...
     xsolu_p= abs(xsolu)
     mxsolu(:,j)= xsolu
     erp(j)=SUM((RMatMul_SP(S_sprs,xsolu_p,nsz)-S_ydata)**2)  !chi2=||As*x-Ys||^2 for solu_j
 ...
 if (nsz/=ndis) STOP "There is an error for wdis and xsolu"
 DO k=1,ndis
    nsolu(iw(k),jw(k),kw(k))=  xsolu_av(k) 
 ENDDO
   write(sn1,'(I3)') i
   sname='Nav-'//cname//csptex//'_'//gridname//'-'//trim(adjustL(sn1))//'.dat'   ! mean Solution for mu_i
 open(unit=10,file=sname,form='unformatted',status="replace",action="write",iostat=open_status)
 if (open_status >0) stop "----Error, File not opened properly." 
   write(10) grid_sz
   write(10) xcar  !double type
   write(10) nsolu*n_unit ! real type (cm^-3)
   close(10)
   print*,'Solution file ['//sname//'] is created!'

 DO k=1,ndis
    nsolu(iw(k),jw(k),kw(k))= xsolu_er(k)
 ENDDO

 sname='Ner-'//cname//csptex//'_'//gridname//'-'//trim(adjustL(sn1))//'.dat'   ! standard deviation for mu_i
 open(unit=10,file=sname,form='unformatted',status="replace",action="write",iostat=open_status)
 if (open_status >0) stop "----Error, File not opened properly."
   write(10) grid_sz
   write(10) xcar
