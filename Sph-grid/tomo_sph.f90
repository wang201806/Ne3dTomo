...
!  Main program
! history: 
! Final version for Spheric grid code, modified on 03/18/2025 by Tongjiang Wang
!  the solution of electron density in units of cm^-3
! This version has an option for CG using the zeroth-order regularization
!    
program tomo_sph
 use InternPro
 use TomoPro
...
!                            isz=16 for grid: sz_r=6, sz_th=26, sz_ph=51
integer, parameter :: nimg=60, isz=128   !for grid: sz_r=51, sz_th=91, sz_ph=181
...
type (dataRys) :: arrRys(nimg*isz*isz)
...
real (kind=double), dimension(:), allocatable :: rp1d, nrw
integer :: sav_res, sav_iter, resume, reg0
...
write(sn3,'(I3)') sz_r
 ...
 if (wt_nr == 1) then
     ...
     if (reg0 == 1) then
          Rbuffname='Buff_Rsp_reg0_' // gridname // '_nwt.dat'
          namesolu='nsolu_'// gridname //'_reg0_nwt.dat'
     endif
 else
     Rbuffname='Buff_Rsp_reg2_' // gridname // '.dat'
     namesolu='nsolu_'// gridname //'_reg2.dat'
     if (reg0 == 1) then
          Rbuffname='Buff_Rsp_reg0_' // gridname // '.dat'
          namesolu='nsolu_'// gridname //'_reg0.dat'
     endif
 endif

 Abuffname='Buff_Asp_' // gridname //'.dat'
...
! nn=         !set initial value for row number of A_sparse
...
!        the angle between OP and LOS, and vec_ni is the unit vector of LOS.
       ...
       ELSE
             STOP "---error with sub: get_cross_points"// &
                "All rays should cross the Domain!"
       ENDIF cross_if
...
ENDIF
 ...
 IF (wt_nr ==1) THEN
  ...
  ! use 0-order regularization with wt=1/n(r)
     if (reg0 ==1) then
         allocate(rp1d((sz_ph-1)*sz_th*sz_r), nrw((sz_ph-1)*sz_th*sz_r))
         rp1d=reshape(rp, shape(rp1d))
         call interp(nr(:,1),nr(:,2), rp1d,nrw)
     endif
 ELSE
    ALLOCATE(nr(1,2), nrw(1))
    nr=0.d0
    nrw=0.d0
...
!  save Data from sph_Reg2nd2smtp_SP() for regularization R_sparse in a buff file
...
! xsolu= ConjugateGrad_SP (Amat,ydata,mu,Rmat, tol,sav_res,sav_iter,resume)
 xsolu= ConjugateGrad_SP_reg0 (Amat,ydata,mu,Rmat, tol,sav_res,sav_iter,resume, reg0,nrw)
...
! ; return solution with ph=[0,2*pi], th=[-pi/2, pi/2], r=[Rin. Rout]
 ...
 INQUIRE(FILE=namesolu, exist=exist)
  ...
  if (Openstatus >0) stop "----Error, File not opened properly. &
   ...
   nsolu= nsolu*n_unit   ! electron density in cm^-3              
...
print*,'Total computing time=',tfinish-tstart

end program tomo_sph
